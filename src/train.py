...
def train_model(X_train, X_test, y_train, y_test, modelpath_prefix, lr=0.001):
    ...
    model.compile(loss='mse', optimizer=Adam(lr=lr, decay=1e-6, clipnorm=1.))

    reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.2,
                                  patience=5, min_lr=0.00001)

    # checkpoint
    filepath = f"{modelpath_prefix}_-in{HISTORY}-out{FORECAST}.hdf5"
...
if __name__ == "__main__":
    prefix = 'production'
    train, test = split_data(production)
    ...
    train_model(X_train, X_test, y_train, y_test, prefix, lr=0.001)
